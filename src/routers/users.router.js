...
router.delete('/users/me', auth, async (req, res) => {
    try {
        await req.user.remove(); //have access to req.user becasue of the auth middleware
...
router.post('/users/me/avatar', auth, upload.single('avatar'), async (req, res) => {
    const buffer = await sharp(req.file.buffer).resize({ width: 250, height: 250 }).png().toBuffer(); //buffer of the modified image file
